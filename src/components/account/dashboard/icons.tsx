/**
 * Inline heroicons-style SVGs for the account dashboard (DESIGN.md §4: stroke
 * currentColor, aria-hidden by the caller's context). Moved out of
 * subscribe-content.tsx so the dashboard cards can share one set.
 */
...
export const featureIcons: React.ReactNode[] = [
...
export function ChevronBackIcon({ className = 'w-4 h-4' }: { className?: string }) {
  return (
    <svg className={`rtl:-scale-x-100 ${className}`} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" aria-hidden="true">
...
/** Same mapping as the apps (DeviceSessionService.swift typeIcon): phone, laptop, desktop, browser. */
export function DeviceTypeIcon({ type, className = 'w-5 h-5' }: { type: string | null; className?: string }) {
  const t = (type ?? '').toLowerCase();
  let d: string;
  if (t === 'ios' || t === 'android') {
    // device-phone-mobile
    d = 'M10.5 1.5H8.25A2.25 2.25 0 0 0 6 3.75v16.5a2.25 2.25 0 0 0 2.25 2.25h7.5A2.25 2.25 0 0 0 18 20.25V3.75a2.25 2.25 0 0 0-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-3 18.75h3';
  } else if (t === 'chrome' || t === 'firefox' || t === 'web') {
    // globe-alt
    d = 'M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418';
  } else {
    // computer-desktop: macOS, Windows, and anything unknown (heroicons has no laptop)
...
export function EllipsisIcon({ className = 'w-5 h-5' }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" aria-hidden="true">
