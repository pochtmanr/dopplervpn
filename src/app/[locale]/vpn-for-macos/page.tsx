...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "vpnForMacos.metadata" });
  const title = t("title");
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${baseUrl}/${locale}/vpn-for-macos`,
      languages: Object.fromEntries([
        ...routing.locales.map((loc) => [loc, `${baseUrl}/${loc}/vpn-for-macos`]),
        ["x-default", `${baseUrl}/en/vpn-for-macos`],
      ]),
    },
    openGraph: {
      title,
      description,
      url: `${baseUrl}/${locale}/vpn-for-macos`,
...
export default async function VpnForMacosPage({ params }: PageProps) {
  ...
  const t = await getTranslations("vpnForMacos");
  const mt = await getTranslations({ locale, namespace: "vpnForMacos.metadata" });
    ...
    <>
      <BreadcrumbSchema
        items={[
          ...
          { name: t("hero.title"), url: `${baseUrl}/${locale}/vpn-for-macos` },
        ]}
      />
      <PlatformAppSchema
        name="Doppler VPN"
              ...
              <div className="hero-animate hero-animate-delay-3 relative flex justify-center lg:justify-end">
                ...
                <div className="relative w-full max-w-lg lg:max-w-xl aspect-[4/3] rounded-[2rem] ring-1 ring-overlay/10 shadow-2xl shadow-black/40 bg-gradient-to-br from-accent-teal/20 via-bg-secondary to-accent-gold/10 overflow-hidden flex items-center justify-center">
                  <div
                    ...
                    aria-hidden="true"
                  />
                  <div className="absolute w-48 h-48 bg-accent-teal/20 rounded-full blur-2xl" aria-hidden="true" />
                  <AppleIcon className="relative w-32 h-32 text-text-muted/40" />
                </div>
              </div>
            </div>
          </div>
      ...
      <BlogStickyBar sentinelId="blog-cta-sentinel" trackingLocation="vpn-for-macos" />
      <Footer />
    </>
  );
}
